...
export function RankingLeadsFechados({ leads }: Props) {
  const rows = useMemo(() => {
  ...
  if (rows.length === 0) {
    return (
      <div className="card-glass p-4 rounded-xl">
        <h3 className="text-sm font-semibold text-foreground mb-3">Leads Fechados</h3>
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-border">
            ...
            <th className="text-right py-2 text-muted-foreground font-medium">Leads Fechados</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((r) => (
